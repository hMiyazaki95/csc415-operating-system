...
#define DEBUG
	...
	// Initialize bufLoc pointer to the end of the buffer to indicate we
...
int b_read (b_io_fd fd, char * buffer, int count)
  ...
  // Count the number of bytes actually copied into buffer
	int retCount = 0;
	// Keep track of where in the buffer we need to copy blocks to
...
#define NUM_OF_CHARS_REMAIN_IN_CHUNKBUF \
  ...
  // If count is asking for more than we have in chunkBuf,
	// we need to get more blocks.
  while (count > NUM_OF_CHARS_REMAIN_IN_CHUNKBUF)
		{
		  ...
		  // Next copy from chunkBuf to buffer starts at the end of what we
			// just copied
			outBuffer += len;
			count -= len;
			retCount += len;
		...
		// Return what we actually copied
#ifdef DEBUG
		fcbArray[fd].requestCount += retCount;
		fprintf(debug[fd],
