...
dispatch_t dispatchTable[] = {
	...
	{"mkdir",   cmd_md, 	 "Make a new directory (alt syntax)"}, // Added
	...
	while (di != NULL) 
	{
		if ((di->d_name[0] != '.') || (flall)) // if not all and starts with '.' it is hidden
		{
			if (fllong)
			{
				...
				if (fs_stat(di->d_name, &statbuf) == 0)
				{
					printf("%s    %9ld  %9ld  %9ld   %s\n",
						fs_isDir(di->d_name) ? "D" : "-", // "D" for directory, else "-"
						statbuf.st_size,			      // Size of this entry in bytes
						statbuf.st_blocks,				  // Size of this entry in blocks
						statbuf.st_createtime,	     	  // Creation time
						di->d_name						  // Entry name
					);
				}
			}
			else
...
#if DEBUG_LS
			printf("=>DEBUG_LS: Processing '%s'\n", argvec[k]);
#endif

			if (fs_isDir(argvec[k]))
			{
				fdDir * dirp = fs_opendir(argvec[k]);
				displayFiles(dirp, flall, fllong);
			}
			else // it is a non-directory file?
			{
...
int cmd_touch(int argcnt, char * argvec[])
{
		...
		{
			printf("Sorry, that name is not allowed\n");
			return 1;
		}

        int testfs_src_fd = b_open(src, O_WRONLY | O_CREAT);
        if (testfs_src_fd < 0)
...
int cmd_cat(int argcnt, char * argvec[])
{
	...
	int testfs_src_fd = b_open(src, O_RDONLY);
...
#if DEBUG_CAT
		printf("=>DEBUG_CAT: just read %d\n", readcnt);
#endif

		buf[readcnt] = '\0';
		printf("%s", buf);
	}
	while (readcnt == BUFFERLEN-1); // Fixed
...
int cmd_cp(int argcnt, char * argvec[])
{
	...
	if (fs_isDir(dest) == 1)
	{
		printf("ERROR: cannot overwrite a directory\n");
		return 1;
	}

	int testfs_src_fd = b_open(src, O_RDONLY);
	int testfs_dest_fd = b_open(dest, O_WRONLY | O_CREAT | O_TRUNC);
...
#if DEBUG_CP
		printf("=>DEBUG_CP: readcnt is %d\n", readcnt);
#endif

		writecnt = b_write(testfs_dest_fd, buf, readcnt);
	}
	while(readcnt == BUFFERLEN && writecnt != -1);		

	b_close(testfs_src_fd);
	int closeRet = b_close(testfs_dest_fd);

	if (writecnt == -1 || closeRet == -1)
		fs_delete(dest);
#endif
	return 0;
}
...
int cmd_mv(int argcnt, char * argvec[])
{
	...
	// First copy the file
	if (cmd_cp(argcnt, argvec) != 0)
		return 1;

	// Then delete the source file. While the length of argvec
	// is actually 3, the 3rd param will be ignored so no problem
	int ret = cmd_rm(2, argvec);
...
int cmd_md(int argcnt, char * argvec[])
{
	...
	if (argcnt != 2)
	{
		printf("Usage: md/mkdir pathname\n");
		return -1;
	}
	else
	{
...
int cmd_rm(int argcnt, char * argvec[])
{
	...
	{
#if DEBUG_RM
		printf("=>DEBUG_RM: doing fs_rmdir\n");
#endif
	...
	{
#if DEBUG_RM
		printf("=>DEBUG_RM: doing fs_delete\n");
#endif
...
int cmd_cp2l(int argcnt, char * argvec[])
{
	...
	int testfs_fd = b_open(src, O_RDONLY);
	int linux_fd = open(dest, O_WRONLY | O_CREAT | O_TRUNC, PERMISSIONS);
...
int cmd_cp2fs(int argcnt, char * argvec[])
{
	...
	// Make sure that the linux file exists
	int linux_fd = open(src, O_RDONLY);
	if (linux_fd < 0)
	{
		printf("ERROR: Linuxsrcfile has to exist\n");
		return 1;
	}
	
	int testfs_fd = b_open(dest, O_WRONLY | O_CREAT | O_TRUNC);

	do 
	{
		readcnt = read(linux_fd, buf, BUFFERLEN);
		writecnt = b_write(testfs_fd, buf, readcnt);
	}
	while (readcnt == BUFFERLEN && writecnt != -1);

	b_close(testfs_fd);
	int closeRet = close(linux_fd);
...
int cmd_cd(int argcnt, char * argvec[])
{
	...
	char * path = argvec[1];	// new directory path
...
int main (int argc, char * argv[])
{
	...
	strncpy(g_currDir, "/", 1); // Start with cwd = rootdir
	...
	while (1)
	{
		char promptTxt[DIRMAX_LEN + 2];
		strcpy(promptTxt, g_currDir);
		strcat(promptTxt, "$ ");
		cmdin = readline(promptTxt);
